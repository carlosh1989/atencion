...
//alert(tipo_id);
$.get("<?php echo baseUrl ?>fundacion/solicitantesFundacion/parroquias", { idMunicipio:idMunicipio }, function(data){
...
//alert(inputEmail);
$.get("<?php echo baseUrl ?>fundacion/solicitantesFundacion/verificar_email", { email:email }, function(existe){
if(existe == true) {
swal(
'Error...',
    ...
    <div class="row">
      <div class="col-md-12">
        <form id="formulario" method="GET" action="">
          ...
          <div class="col-md-3">
            <button class="btn btn-danger fa fa-search btn-lg"></button>
          </div>
        </form>
      </div>
    </div>
    <br>
    <h5 class="text-muted text-danger text-center text-uppercase">
    <i class="fa fa-folder-open-o" aria-hidden="true"></i>
    <?php if (isset($tipo_seleccion) and $tipo_seleccion): ?>
    <a class="text-danger"><?php echo $tipo_seleccion->nombre ?></a>
    <?php else: ?>
    SOLICITUDES
    <?php endif ?>
    </h5>
          ...
          <tbody>
            ...
            <tr id="tr<?php echo $c->id ?>">
              ...
              <td class="text-uppercase text-center">
                ...
                ?>
                <button class="btn btn-danger"><?php echo $dias ?></button>
              </td>
              <td class="text-uppercase">
                <a class="btn btn-default" href="<?php echo baseUrl ?>fundacion/solicitudesFundacion/<?php echo $c->id ?>"><i class="fa fa-search text-danger"></i></a>
              </td>
              <td>
                <?php if ($c->tipo_solicitud->id == 1): ?>
                <form action="<?php echo baseUrl ?>fundacion/solicitudesFundacion/monto" method="GET">
                ...
                <!--                 <form action="<?php echo baseUrl ?>admin/solicitudes/aprobar">
                  ...
                  var estatus = 2;
                  $.get("<?php echo baseUrl ?>fundacion/solicitudesFundacion/estatus", { solicitud_id:solicitudID, estatus:estatus}, function(data){
                  ...
                  $.get("<?php echo baseUrl ?>fundacion/solicitudesFundacion/estatus", { solicitud_id:solicitudID, estatus:estatus, observacion:observacion }, function(data){
                  //alert(JSON.stringify(data));
                  });
                  $("#tr<?php echo $c->id ?>").remove();
                  })
